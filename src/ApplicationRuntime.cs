    ...
    public class ApplicationRuntime : IAsyncDisposable
    {
        ...
        public Dictionary<string, string> GetSystemState()
        {
            return new()
            {
                ...
                ["salinity"] = double.IsNaN(Salinity) ? "0.0" : Salinity.ToString("F1"),
                ["max_distance"] = double.IsNaN(MaxDistance) ? "0" : MaxDistance.ToString("F0"),
                ["sound_speed"] = double.IsNaN(SoundSpeed) ? "" : SoundSpeed.ToString("F1"),
                ["antenna_x"] = double.IsNaN(AntennaXOffset) ? "0.00" : AntennaXOffset.ToString("F2"),
                ["antenna_y"] = double.IsNaN(AntennaYOffset) ? "0.00" : AntennaYOffset.ToString("F2"),
                ["antenna_phi"] = double.IsNaN(AntennaPhi) ? "0.0" : AntennaPhi.ToString("F1"),
